...
namespace TotalDAL.Helpers.SqlProgrammability.Inventories
{
    public class Inventory
    {
        ...
        {//pendingForecast: 0, 1: 
            ...
            queryString = queryString + "           DECLARE     @ForecastEntryDate DateTime, @IsCurrentForecastEntryDate int        SET @ForecastEntryDate = DATEFROMPARTS (YEAR(@ToDate), MONTH(@ToDate), 1)          SET @IsCurrentForecastEntryDate = 0 " + "\r\n";
            queryString = queryString + "           IF          (NOT EXISTS(SELECT TOP 1 ForecastDetailID FROM ForecastDetails WHERE EntryDate = @ForecastEntryDate))  BEGIN     SET @ForecastEntryDate = DATEADD(MONTH, -1, @ForecastEntryDate)    SET @IsCurrentForecastEntryDate = 1       END " + "\r\n";
        ...
        private string DEFINEWarehouseJournal(bool isQ, bool isLocationID, bool isWarehouseID, bool isPending, bool isForecast, bool isCommodityCategoryID, bool isCommodityID, bool isCommodityTypeID, bool isSameFromToDate)
        {
            ...
            if (isPending)
            {
                ...
                queryString = queryString + "               WHERE   SalesOrderDetails.EntryDate <= @ToDate  AND SalesOrderDetails." + Quantity(isQ) + " > SalesOrderDetails." + Quantity(isQ) + "Advice AND SalesOrderDetails.InActive = 0 AND SalesOrderDetails.InActivePartial = 0 " + this.DEFINEFilterLW(isLocationID, false, "SalesOrderDetails") + "\r\n";
                ...
                queryString = queryString + "               WHERE   DeliveryAdviceDetails.EntryDate <= @ToDate  AND DeliveryAdviceDetails." + Quantity(isQ) + " > DeliveryAdviceDetails." + Quantity(isQ) + "Issue AND DeliveryAdviceDetails.InActive = 0 AND DeliveryAdviceDetails.InActivePartial = 0 " + this.DEFINEFilterLW(isLocationID, false, "DeliveryAdviceDetails") + "\r\n";
                ...
                //--PENDING DELIVERYADVICE.BEGIN
                queryString = queryString + "               UNION ALL " + "\r\n";
                queryString = queryString + "               SELECT  NULL AS EntryDate, NULL AS GoodsReceiptDetailID, TransferOrderDetails.CommodityID, NULL AS BatchEntryDate, TransferOrderDetails.LocationID, NULL AS WarehouseID, NULL AS BinLocationID, NULL AS PickupID, NULL AS GoodsIssueID, NULL AS WarehouseAdjustmentID, NULL AS PackID, NULL AS CartonID, NULL AS PalletID, " + "\r\n";
                queryString = queryString + "                       0 AS " + Quantity(isQ) + "Begin, 0 AS " + Quantity(isQ) + "ReceiptPickup, 0 AS " + Quantity(isQ) + "ReceiptPurchasing, 0 AS " + Quantity(isQ) + "ReceiptTransfer, 0 AS " + Quantity(isQ) + "ReceiptReturn, 0 AS " + Quantity(isQ) + "ReceiptAdjustment, 0 AS " + Quantity(isQ) + "IssueSelling, 0 AS " + Quantity(isQ) + "IssueTransfer, 0 AS " + Quantity(isQ) + "IssueProduction, 0 AS " + Quantity(isQ) + "IssueAdjustment, 0 AS " + Quantity(isQ) + "OnPurchasing, 0 AS " + Quantity(isQ) + "OnPickup, 0 AS " + Quantity(isQ) + "OnTransit, 0 AS " + Quantity(isQ) + "PendingOrder, (TransferOrderDetails." + Quantity(isQ) + " - TransferOrderDetails." + Quantity(isQ) + "Issue) AS " + Quantity(isQ) + "PendingAdvice, 0 AS " + Quantity(isQ) + "Forecasts, 0 AS " + Quantity(isQ) + "M1Forecasts, 0 AS " + Quantity(isQ) + "M2Forecasts, 0 AS " + Quantity(isQ) + "M3Forecasts, 0 AS MovementMIN, 0 AS MovementMAX, 0 AS MovementAVG " + "\r\n";
                queryString = queryString + "               FROM    TransferOrderDetails " + "\r\n";
                queryString = queryString + "               WHERE   TransferOrderDetails.EntryDate <= @ToDate  AND TransferOrderDetails." + Quantity(isQ) + " > TransferOrderDetails." + Quantity(isQ) + "Issue AND TransferOrderDetails.InActive = 0 AND TransferOrderDetails.InActivePartial = 0 " + this.DEFINEFilterLW(isLocationID, false, "TransferOrderDetails") + "\r\n";

                queryString = queryString + "               UNION ALL " + "\r\n";
                queryString = queryString + "               SELECT  NULL AS EntryDate, NULL AS GoodsReceiptDetailID, GoodsIssueDetails.CommodityID, NULL AS BatchEntryDate, GoodsIssueDetails.LocationID, NULL AS WarehouseID, NULL AS BinLocationID, NULL AS PickupID, NULL AS GoodsIssueID, NULL AS WarehouseAdjustmentID, NULL AS PackID, NULL AS CartonID, NULL AS PalletID, " + "\r\n";
                queryString = queryString + "                       0 AS " + Quantity(isQ) + "Begin, 0 AS " + Quantity(isQ) + "ReceiptPickup, 0 AS " + Quantity(isQ) + "ReceiptPurchasing, 0 AS " + Quantity(isQ) + "ReceiptTransfer, 0 AS " + Quantity(isQ) + "ReceiptReturn, 0 AS " + Quantity(isQ) + "ReceiptAdjustment, 0 AS " + Quantity(isQ) + "IssueSelling, 0 AS " + Quantity(isQ) + "IssueTransfer, 0 AS " + Quantity(isQ) + "IssueProduction, 0 AS " + Quantity(isQ) + "IssueAdjustment, 0 AS " + Quantity(isQ) + "OnPurchasing, 0 AS " + Quantity(isQ) + "OnPickup, 0 AS " + Quantity(isQ) + "OnTransit, 0 AS " + Quantity(isQ) + "PendingOrder, GoodsIssueDetails." + Quantity(isQ) + " AS " + Quantity(isQ) + "PendingAdvice, 0 AS " + Quantity(isQ) + "Forecasts, 0 AS " + Quantity(isQ) + "M1Forecasts, 0 AS " + Quantity(isQ) + "M2Forecasts, 0 AS " + Quantity(isQ) + "M3Forecasts, 0 AS MovementMIN, 0 AS MovementMAX, 0 AS MovementAVG " + "\r\n";
                queryString = queryString + "               FROM    TransferOrders INNER JOIN " + "\r\n";
                queryString = queryString + "                       GoodsIssueDetails ON TransferOrders.TransferOrderID = GoodsIssueDetails.TransferOrderID AND TransferOrders.EntryDate <= @ToDate  AND GoodsIssueDetails.EntryDate > @ToDate  " + this.DEFINEFilterLW(isLocationID, false, "GoodsIssueDetails") + "\r\n";
                //--PENDING DELIVERYADVICE.END
            }
            ...
            if (isForecast)
            {
                ...
                queryString = queryString + "                       0 AS " + Quantity(isQ) + "Begin, 0 AS " + Quantity(isQ) + "ReceiptPickup, 0 AS " + Quantity(isQ) + "ReceiptPurchasing, 0 AS " + Quantity(isQ) + "ReceiptTransfer, 0 AS " + Quantity(isQ) + "ReceiptReturn, 0 AS " + Quantity(isQ) + "ReceiptAdjustment, 0 AS " + Quantity(isQ) + "IssueSelling, 0 AS " + Quantity(isQ) + "IssueTransfer, 0 AS " + Quantity(isQ) + "IssueProduction, 0 AS " + Quantity(isQ) + "IssueAdjustment, 0 AS " + Quantity(isQ) + "OnPurchasing, 0 AS " + Quantity(isQ) + "OnPickup, 0 AS " + Quantity(isQ) + "OnTransit, 0 AS " + Quantity(isQ) + "PendingOrder, 0 AS " + Quantity(isQ) + "PendingAdvice, IIF(@IsCurrentForecastEntryDate = 0, ForecastDetails." + Quantity(isQ) + ", ForecastDetails." + Quantity(isQ) + "M1) AS " + Quantity(isQ) + "Forecasts, IIF(@IsCurrentForecastEntryDate = 0, ForecastDetails." + Quantity(isQ) + "M1, ForecastDetails." + Quantity(isQ) + "M2) AS " + Quantity(isQ) + "M1Forecasts, IIF(@IsCurrentForecastEntryDate = 0, ForecastDetails." + Quantity(isQ) + "M2, ForecastDetails." + Quantity(isQ) + "M3) AS " + Quantity(isQ) + "M2Forecasts, IIF(@IsCurrentForecastEntryDate = 0, ForecastDetails." + Quantity(isQ) + "M3, 0) AS " + Quantity(isQ) + "M3Forecasts, 0 AS MovementMIN, 0 AS MovementMAX, 0 AS MovementAVG " + "\r\n";
                queryString = queryString + "               FROM    ForecastDetails " + "\r\n"; //FORECAST: LẤY THÁNG BÁO CÁO || HOẶC LÀ THÁNG LIỀN KỀ TRƯỚC ĐÓ. THEO NGUYÊN LÝ NÀY: NẾU LẤY THÁNG BÁO CÁO: THÌ LẤ ĐÚNG COLUMN: Quantity, QuantityM1, QuantityM2, QuantityM3. NẾU LẤY THÁNG LIỀN KỀ: THÌ DỊCH CHUYỂN 1 THÁNG TỨC: QuantityM1 => Quantity, QuantityM2 => QuantityM1, QuantityM3 = > QuantityM2, 0 => QuantityM3 CĂN CỨ THEO: IIF(@IsCurrentForecastEntryDate = 0, ....
                queryString = queryString + "               WHERE   ForecastDetails.EntryDate = @ForecastEntryDate " + this.DEFINEFilterLW(isLocationID, false, "ForecastDetails", "ForecastLocationID") + "\r\n"; //ForecastDetails.EntryDate = @ForecastEntryDate: @ForecastEntryDate ĐƯỢC TÍNH TOÁN TỪ TRƯỚC: THÁNG BÁO CÁO @ToDate || HOẶC THÁNG LIỀN KỀ TRƯỚC ĐÓ DATEADD(MONTH, -1, @ToDate) 
                //--FORECAST.END
            }
