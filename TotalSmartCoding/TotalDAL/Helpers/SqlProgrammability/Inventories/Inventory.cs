...
namespace TotalDAL.Helpers.SqlProgrammability.Inventories
{
    public class Inventory
    {
        ...
        private string WFUpdate_L_H_A_DIOH(bool isLocationID)
        {
            ...
            queryString = queryString + "       SET         WarehouseForecasts.DIOHLow = CommoditySettings.DIOHLow, WarehouseForecasts.DIOHHigh = CommoditySettings.DIOHHigh, WarehouseForecasts.DIOHAlert = CommoditySettings.DIOHAlert " + "\r\n";
            queryString = queryString + "       FROM        @WarehouseForecasts WarehouseForecasts " + "\r\n";
            queryString = queryString + "                   LEFT JOIN (SELECT CommodityID, LocationID, SUM(DIOHLow) AS DIOHLow, SUM(DIOHHigh) AS DIOHHigh, SUM(DIOHAlert) AS DIOHAlert FROM CommoditySettings " + (isLocationID ? " WHERE LocationID IN (SELECT Id FROM dbo.SplitToIntList (@LocationIDs)) " : "") + " GROUP BY CommodityID, LocationID) CommoditySettings ON WarehouseForecasts.CommodityID = CommoditySettings.CommodityID AND WarehouseForecasts.LocationID = CommoditySettings.LocationID " + "\r\n";
