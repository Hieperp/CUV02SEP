...
namespace TotalDAL.Helpers.SqlProgrammability.Inventories
{
    public class GoodsReceipt
    {
        ...
        public void RestoreProcedure()
        {
            ...
            GenerateSQLPendingDetails generatePendingSalesReturnDetails = new GenerateSQLPendingDetails(totalSmartCodingEntities, GlobalEnums.GoodsReceiptTypeID.SalesReturn, "SalesReturns", "SalesReturnDetails", "SalesReturnID", "@SalesReturnID", "SalesReturnDetailID", "@SalesReturnDetailIDs", "WarehouseID", "PrimaryReference", "PrimaryEntryDate");
            generatePendingSalesReturnDetails.GetPendingPickups("GetPendingSalesReturns");
            generatePendingSalesReturnDetails.GetPendingPickupWarehouses("GetPendingSalesReturnWarehouses");
            generatePendingSalesReturnDetails.GetPendingPickupDetails("GetPendingSalesReturnDetails");
        ...
        private void GetGoodsReceiptIndexes()
        {
            ...
            queryString = queryString + "                   LEFT JOIN SalesReturns ON GoodsReceipts.SalesReturnID = SalesReturns.SalesReturnID " + "\r\n";
            queryString = queryString + "                   LEFT JOIN Customers ON SalesReturns.CustomerID = Customers.CustomerID " + "\r\n";
        ...
        private void GetGoodsReceiptViewDetails()
        {
            ...
            queryString = queryString + "                   GoodsReceiptDetails.SalesReturnID, GoodsReceiptDetails.SalesReturnDetailID, SalesReturnDetails.Reference AS SalesReturnReference, SalesReturnDetails.EntryDate AS SalesReturnEntryDate, " + "\r\n";
            ...
            queryString = queryString + "                   LEFT JOIN SalesReturnDetails ON GoodsReceiptDetails.SalesReturnDetailID = SalesReturnDetails.SalesReturnDetailID " + "\r\n";
        ...
        private void GoodsReceiptSaveRelative()
        {
            ...
            queryString = queryString + "                   IF (@GoodsReceiptTypeID = " + (int)GlobalEnums.GoodsReceiptTypeID.SalesReturn + ") " + "\r\n";
            queryString = queryString + "                       BEGIN  " + "\r\n";
            queryString = queryString + "                           UPDATE          SalesReturnDetails " + "\r\n";
            queryString = queryString + "                           SET             SalesReturnDetails.QuantityReceipt = ROUND(SalesReturnDetails.QuantityReceipt + GoodsReceiptDetails.Quantity * @SaveRelativeOption, " + (int)GlobalEnums.rndQuantity + "), SalesReturnDetails.LineVolumeReceipt = ROUND(SalesReturnDetails.LineVolumeReceipt + GoodsReceiptDetails.LineVolume * @SaveRelativeOption, " + (int)GlobalEnums.rndVolume + ") " + "\r\n";
            queryString = queryString + "                           FROM            GoodsReceiptDetails " + "\r\n";
            queryString = queryString + "                                           INNER JOIN SalesReturnDetails ON GoodsReceiptDetails.GoodsReceiptID = @EntityID AND SalesReturnDetails.Approved = 1 AND GoodsReceiptDetails.SalesReturnDetailID = SalesReturnDetails.SalesReturnDetailID " + "\r\n";
            queryString = queryString + "                           SET @AffectedROWCOUNT = @@ROWCOUNT " + "\r\n";
            queryString = queryString + "                       END " + "\r\n";
        ...
        private void GoodsReceiptPostSaveValidate()
        {
            ...
            queryArray[2] = " SELECT TOP 1 @FoundEntity = N'Ngày trả hàng: ' + CAST(SalesReturns.EntryDate AS nvarchar) FROM GoodsReceiptDetails INNER JOIN SalesReturns ON GoodsReceiptDetails.GoodsReceiptID = @EntityID AND GoodsReceiptDetails.SalesReturnID = SalesReturns.SalesReturnID AND GoodsReceiptDetails.EntryDate < SalesReturns.EntryDate ";
            queryArray[3] = " SELECT TOP 1 @FoundEntity = N'Số lượng nhập kho vượt quá số lượng giao: ' + CAST(ROUND(Quantity - QuantityReceipt, " + (int)GlobalEnums.rndQuantity + ") AS nvarchar) FROM PickupDetails WHERE (ROUND(Quantity - QuantityReceipt, " + (int)GlobalEnums.rndQuantity + ") < 0) ";
            queryArray[4] = " SELECT TOP 1 @FoundEntity = N'Số lượng nhập kho vượt quá số lượng giao: ' + CAST(ROUND(Quantity - QuantityReceipt, " + (int)GlobalEnums.rndQuantity + ") AS nvarchar) FROM GoodsIssueTransferDetails WHERE (ROUND(Quantity - QuantityReceipt, " + (int)GlobalEnums.rndQuantity + ") < 0) ";
            queryArray[5] = " SELECT TOP 1 @FoundEntity = N'Số lượng nhập kho vượt quá số lượng trả hàng: ' + CAST(ROUND(Quantity - QuantityReceipt, " + (int)GlobalEnums.rndQuantity + ") AS nvarchar) FROM SalesReturnDetails WHERE (ROUND(Quantity - QuantityReceipt, " + (int)GlobalEnums.rndQuantity + ") < 0) ";
        ...
        private string AvailableBuildSQLGoodsReceiptDetailIDs(bool isWarehouseID, bool isCommodityID, bool isCommodityIDs, bool isBatchID, bool isGoodsReceiptDetailIDs)
        {
            ...
            queryString = queryString + "                   ISNULL('Production: ' + ' ' + Pickups.Reference, ISNULL('From: ' + ' ' + SourceWarehouses.Name + ', ' + GoodsIssues.VoucherCodes, ISNULL(WarehouseAdjustmentTypes.Name  + ' ' + WarehouseAdjustments.AdjustmentJobs, ISNULL('Return: ' + ' ' + SalesReturns.Reference, '')))) AS LineReferences, GoodsReceiptDetails.Approved, Warehouses.Issuable, " + "\r\n";
            ...
            queryString = queryString + "                   LEFT JOIN SalesReturns ON GoodsReceiptDetails.SalesReturnID = SalesReturns.SalesReturnID " + "\r\n";
        ...
        private class GenerateSQLPendingDetails
        {
            ...
            public void GetPendingPickups(string myfunctionName)
            {
                ...
                queryString = queryString + "       SELECT          " + this.primaryTables + "." + this.primaryKey + ", " + this.primaryTables + ".Reference AS " + this.primaryReference + ", " + this.primaryTables + ".EntryDate AS " + this.primaryEntryDate + ", Warehouses.WarehouseID, Warehouses.Code AS WarehouseCode, Warehouses.Name AS WarehouseName, " + (int)this.goodsReceiptTypeID + " AS GoodsReceiptTypeID, (SELECT TOP 1 Name FROM GoodsReceiptTypes WHERE GoodsReceiptTypeID = " + (int)this.goodsReceiptTypeID + ") AS GoodsReceiptTypeName, " + this.primaryTables + ".Description, " + this.primaryTables + ".Remarks " + (this.goodsReceiptTypeID == GlobalEnums.GoodsReceiptTypeID.GoodsIssueTransfer ? ", GoodsIssues.VoucherCodes, SourceWarehouses.Name AS SourceWarehouseName " : "") + (this.goodsReceiptTypeID == GlobalEnums.GoodsReceiptTypeID.SalesReturn ? ", Customers.Code + '-' + Customers.Name AS CustomerName " : "") + "\r\n";
                queryString = queryString + "       FROM           (SELECT " + this.primaryKey + ", " + fieldNameWarehouseID + (this.goodsReceiptTypeID == GlobalEnums.GoodsReceiptTypeID.GoodsIssueTransfer && fieldNameWarehouseID.IndexOf("WarehouseID") == -1 ? ", WarehouseID" : "") + (this.goodsReceiptTypeID == GlobalEnums.GoodsReceiptTypeID.SalesReturn ? ", CustomerID " : "") + ", Reference, EntryDate, Description, Remarks" + (this.goodsReceiptTypeID == GlobalEnums.GoodsReceiptTypeID.GoodsIssueTransfer ? ", VoucherCodes" : "") + " FROM " + this.primaryTables + " WHERE " + this.primaryKey + " IN (SELECT " + this.primaryKey + " FROM " + this.primaryTableDetails + " WHERE LocationID = @LocationID AND Approved = 1 AND ROUND(Quantity - QuantityReceipt, " + (int)GlobalEnums.rndQuantity + ") > 0)) AS " + this.primaryTables + "\r\n";
                ...
                if (this.goodsReceiptTypeID == GlobalEnums.GoodsReceiptTypeID.SalesReturn)
            ...
            private string BuildSQLEdit(bool isPickupID, bool isPickupDetailIDs)
            {
                ...
                queryString = queryString + "       SELECT      " + this.primaryTableDetails + "." + this.primaryKey + ", " + this.primaryTableDetails + "." + this.primaryKeyDetail + ", " + this.primaryTableDetails + ".Reference " + (this.goodsReceiptTypeID == GlobalEnums.GoodsReceiptTypeID.GoodsIssueTransfer ? " + ' [' + GoodsIssues.VoucherCodes + ']' " : "") + " AS " + this.primaryReference + ", " + this.primaryTableDetails + ".EntryDate AS " + this.primaryEntryDate + ", " + "\r\n";
                queryString = queryString + "                   " + (this.goodsReceiptTypeID == GlobalEnums.GoodsReceiptTypeID.GoodsIssueTransfer ? this.primaryTableDetails + ".LocationIssueID + 0" : "NULL") + " AS LocationIssueID, " + (this.goodsReceiptTypeID == GlobalEnums.GoodsReceiptTypeID.GoodsIssueTransfer ? this.primaryTableDetails + ".WarehouseID + 0" : "NULL") + " AS WarehouseIssueID, " + (this.goodsReceiptTypeID == GlobalEnums.GoodsReceiptTypeID.WarehouseAdjustments ? this.primaryTableDetails + ".WarehouseAdjustmentTypeID + 0" : "NULL") + " AS WarehouseAdjustmentTypeID, " + "\r\n";
                queryString = queryString + "                   Commodities.CommodityID, Commodities.Code AS CommodityCode, Commodities.Name AS CommodityName, " + this.primaryTableDetails + ".BatchID, " + this.primaryTableDetails + ".BatchEntryDate, GoodsReceiptDetails.BinLocationID, BinLocations.Code AS BinLocationCode, " + this.primaryTableDetails + ".PackID, Packs.Code AS PackCode, " + this.primaryTableDetails + ".CartonID, Cartons.Code AS CartonCode, " + this.primaryTableDetails + ".PalletID, Pallets.Code AS PalletCode, " + "\r\n";
