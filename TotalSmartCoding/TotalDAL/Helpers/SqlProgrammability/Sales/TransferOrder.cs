...
namespace TotalDAL.Helpers.SqlProgrammability.Sales
{
    public class TransferOrder
    {
        ...
        private void GetTransferOrderIndexes()
        {
            ...
            queryString = queryString + "       SELECT      TransferOrders.TransferOrderID, CAST(TransferOrders.EntryDate AS DATE) AS EntryDate, TransferOrders.Reference, TransferOrders.VoucherCode, TransferOrders.DeliveryDate, Locations.Code AS LocationCode, WarehouseIssues.Code AS WarehouseIssueCode, WarehouseIssues.Name AS WarehouseIssueName, WarehouseReceipts.Code AS WarehouseReceiptCode, WarehouseReceipts.Name AS WarehouseReceiptName, TransferOrders.Description, TransferOrders.TotalQuantity, TransferOrders.TotalLineVolume, TransferOrders.Approved " + "\r\n";
            queryString = queryString + "       FROM        TransferOrders " + "\r\n";
            queryString = queryString + "                   INNER JOIN Locations ON TransferOrders.EntryDate >= @FromDate AND TransferOrders.EntryDate <= @ToDate AND TransferOrders.OrganizationalUnitID IN (SELECT AccessControls.OrganizationalUnitID FROM AccessControls INNER JOIN AspNetUsers ON AccessControls.UserID = AspNetUsers.UserID WHERE AspNetUsers.Id = @AspUserID AND AccessControls.NMVNTaskID = " + (int)TotalBase.Enums.GlobalEnums.NmvnTaskID.TransferOrder + " AND AccessControls.AccessLevel > 0) AND Locations.LocationID = TransferOrders.LocationID " + "\r\n";
            queryString = queryString + "                   INNER JOIN Warehouses WarehouseIssues ON TransferOrders.WarehouseIssueID = WarehouseIssues.WarehouseID " + "\r\n";
        ...
        private void GetTransferOrderViewDetails()
        {
            ...
            queryString = queryString + "       DECLARE     @TransferOrderDetails TABLE (TransferOrderID int NOT NULL, TransferOrderDetailID int NOT NULL, EntryDate datetime NOT NULL, LocationID int NOT NULL, WarehouseIssueID int NOT NULL, CommodityID int NOT NULL, Quantity decimal(18, 2) NOT NULL, LineVolume decimal(18, 2) NOT NULL, Remarks nvarchar(100) NULL) " + "\r\n";
            queryString = queryString + "       INSERT INTO @TransferOrderDetails (TransferOrderID, TransferOrderDetailID, EntryDate, LocationID, WarehouseIssueID, CommodityID, Quantity, LineVolume, Remarks) SELECT TransferOrderID, TransferOrderDetailID, EntryDate, LocationID, WarehouseIssueID, CommodityID, Quantity, LineVolume, Remarks FROM TransferOrderDetails WHERE TransferOrderID = @TransferOrderID " + "\r\n";
